"""
B2WZ1 direct-command loco-manipulation sim2real deployment (projected level base frame).

Run:

    python3 deploy/b2wz1_locomanipulation_plb.py <network_interface> deploy/configs/b2wz1_locomanipulation_plb.yaml --mode pd-stand
    python3 deploy/b2wz1_locomanipulation_plb.py <network_interface> deploy/configs/b2wz1_locomanipulation_plb.yaml --mode lock-arm-policy
    python3 deploy/b2wz1_locomanipulation_plb.py <network_interface> deploy/configs/b2wz1_locomanipulation_plb.yaml --mode full-policy
...
class PresampledKeypointsDirectCommandPLBSim:
    ...
    def _sample_new_command(self):
        ...
        self.keypoints_command_plb = self._table[idx].copy().astype(np.float32)
        self._step_in_cycle = 0
        self._has_cmd = True
...
class B2WZ1PLBLocoManipController:
    def __init__(self, cfg_path: str, mode: str):
        ...
        self.ee_ground_z = float(self.cfg.get("ee_ground_z", self.cfg.get("ground_z", 0.0)))
        self.base_height = float(self.cfg.get("base_height", 0.6017))

        self.ee_cmd_sampler = PresampledKeypointsDirectCommandPLBSim(
    ...
    def wait_for_low_state(self):
        print("[B2WZ1-PLB] Waiting for first B2W lowstate...")
        while getattr(self.low_state, "tick", 0) == 0:
            time.sleep(self.control_dt)
        print(f"[B2WZ1-PLB] First lowstate received: tick={self.low_state.tick}")
    ...
    def _init_history(self):
        obs0 = self.build_obs_step(self.ee_cmd_plb_current)
    ...
    def enter_damping_protection_mode(self):
        print("\n" + "!" * 80)
        print("[B2WZ1-PLB][PROTECT] SELECT pressed: entering high-KD damping mode.")
        print("[B2WZ1-PLB][PROTECT] B2W: kp=0, dq=0, tau=0, large kd.")
        print("[B2WZ1-PLB][PROTECT] Z1 : q_cmd=current q, qd=0, tau=0, large kd.")
        print("[B2WZ1-PLB][PROTECT] Press Ctrl-C to quit after the robot is safe.")
        print("!" * 80 + "\n")
        ...
        try:
            ...
            print("[B2WZ1-PLB][PROTECT] Ctrl-C received. Exit damping mode.")

    def zero_torque_state(self):
        print("[B2WZ1-PLB] Zero torque state. Press START to continue.")
        while self.remote_controller.button[KeyMap.start] != 1:
            create_zero_cmd(self.low_cmd)
            self.send_b2w_cmd()
            time.sleep(self.control_dt)
        print("[B2WZ1-PLB] START pressed. Exit zero torque state.")

    def move_b2w_to_pose_policy(self, target_b2w_pos_policy: np.ndarray, duration: float, hold_arm: bool = False):
        print("[B2WZ1-PLB] Moving B2W to target pose...")
        target_b2w_pos_policy = np.asarray(target_b2w_pos_policy, dtype=np.float32).reshape(16,)
        ...
        print("[B2WZ1-PLB] Reached target B2W pose.")

    def hold_arm_default_until_A(self):
        print("[B2WZ1-PLB] Holding arm default pose. Press A to continue.")
        while self.remote_controller.button[KeyMap.A] != 1:
            create_zero_cmd(self.low_cmd)
            self.send_b2w_cmd()
            self.z1.hold_pose_lowcmd(self.default_arm_pos.copy(), self.default_gripper_pos)
            time.sleep(self.control_dt)
        print("[B2WZ1-PLB] A pressed.")

    def hold_all_default_until_A(self):
        print("[B2WZ1-PLB] Holding full default pose. Press A to start main loop.")
        ...
        if self.mode == "pd-stand":
            ...
            ee_cur_plb = self.compute_ee_current_kp_plb()
            self.ee_cmd_plb_current = ee_cur_plb.copy()
        ...
        print("[B2WZ1-PLB] A pressed. Start main loop.")
    ...
    def step(self):
        ...
        if self.mode == "pd-stand":
            self.ee_cmd_plb_current = self.ee_cmd_sampler.command.copy()
        else:
    ...
    def setup(self):
        print("=" * 80)
        print("B2WZ1 PLB direct-command loco-manipulation sim2real")
        print("=" * 80)
    ...
    def run(self):
        ...
        print("[B2WZ1-PLB] Moving arm to default before B2W startup motion...")
        ...
        print("[B2WZ1-PLB] Re-initializing observation history at loop start pose...")
        ...
        ee_cur_init_plb = self.compute_ee_current_kp_plb()

        sample_first = False if self.mode == "pd-stand" else True
        self.ee_cmd_sampler.reset(ee_cur_init_plb, sample_first=sample_first)
        self.ee_cmd_plb_current = self.ee_cmd_sampler.command.copy()
        ...
        print("[B2WZ1-PLB] Main loop started. Press SELECT to enter damping protection mode.")
        ...
        try:
            ...
            print("[B2WZ1-PLB] KeyboardInterrupt received.")
        ...
        try:
            ...
            print(f"[B2WZ1-PLB][EXIT][WARN] Z1 damping command failed: {exc}")

        print("[B2WZ1-PLB] Exit.")
...
def main():
    ...
    controller = B2WZ1PLBLocoManipController(cfg_path=args.config, mode=args.mode)
    controller.run()
